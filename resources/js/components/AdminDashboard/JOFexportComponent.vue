...
<div>
      <v-toolbar flat color="white">
        <v-toolbar-title>JOF Export Status</v-toolbar-title>
        <v-divider class="mx-4" inset vertical></v-divider>
        <v-text-field v-model="search" append-icon="search" label="Search" single-line hide-details></v-text-field>
      <v-spacer></v-spacer>
      <v-dialog v-model="dialog" max-width="500px">
        <template v-slot:activator="{ on }">
            <v-btn color="primary" dark class="mb-2" v-on="on">Export JOF Order</v-btn>
        </template>
         ...
         <v-form ref="form" v-model="valid" lazy-validation  @submit.prevent>
          <v-card>
                ...
                <v-layout wrap>
                  <v-flex xs12>
                    <!-- <v-text-field v-model="editedItem.due_date" label="Due Date"></v-text-field> -->
                            <v-menu v-model="menu"  :close-on-content-click="false" full-width max-width="290" offset-y >
                              <template v-slot:activator="{ on }">
                                <v-text-field clearable readonly label="Start Date" v-on="on"  :value="date"></v-text-field>
                              </template>
            ...
            <v-card-actions>
              ...
              <v-btn color="blue darken-1" text @click="exportpdf">Export</v-btn>
            </v-card-actions>
          </v-card>
             </v-form>
      </v-dialog>
      ...
      <v-data-table :headers="headers" :items="dataItems" :search="search" class="elevation-1" >
        <template v-slot:top>
          <div class="radiOPTS">
            <label class=' bg-primary textpads'>Pending JOF Order</label>
          </div>
        </template>
...
//methods
    ...
    methods: {
      ...
      exportpdf(){
          location.href = "/api/exportpdf/"+this.date;
      }
